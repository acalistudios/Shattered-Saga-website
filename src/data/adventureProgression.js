// Campaign balance metadata. Story content stays in adventures.js; this file
// defines the expected power curve so selection UI and GM prompts can agree.

export const ADVENTURE_PROGRESSION_METADATA = {
  ashveil_keep: {
    campaignOrder: 1,
    tier: 1,
    recommendedLevel: [1, 2],
    prerequisites: [],
    threatProfile: 'intro investigation',
    combatExpectation: 'avoidable',
    difficultyNotes: 'Teach haunted-site investigation, basic combat, and ritual problem solving. Malachar is a ritual threat, not a fair early duel.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: '+1 situational',
      currencyBandCp: [300, 900],
      permanentUnlock: 'Ashveil village discount or holy protection lead',
    },
  },
  saltblood_mines: {
    campaignOrder: 2,
    tier: 1,
    recommendedLevel: [1, 3],
    prerequisites: [],
    threatProfile: 'survival escape',
    combatExpectation: 'optional',
    difficultyNotes: 'Start with deprivation and improvised tools. Reward stealth, lockpicking, leadership, and restraint as strongly as combat.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'mundane restored gear',
      currencyBandCp: [300, 1200],
      permanentUnlock: 'prisoner allies or legal leverage against Redvein traffickers',
    },
  },
  blackroot_hollow: {
    campaignOrder: 3,
    tier: 1,
    recommendedLevel: [2, 3],
    prerequisites: ['ashveil_keep'],
    threatProfile: 'wilderness hazard',
    combatExpectation: 'optional',
    difficultyNotes: 'Use spiders as terrain, fear, venom, and rescue pressure. Direct nest-clearing should be possible but not the only optimal answer.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 1,
      maxGearTier: 'consumable or crafting material',
      currencyBandCp: [250, 900],
      permanentUnlock: 'safe silk access or Mara as a wilderness contact',
    },
  },
  thorn_treaty: {
    campaignOrder: 4,
    tier: 1,
    recommendedLevel: [2, 4],
    prerequisites: ['saltblood_mines'],
    threatProfile: 'social wilderness',
    combatExpectation: 'dangerous',
    difficultyNotes: 'Make treaty work, restitution, and forest law the primary difficulty. Violence should solve symptoms while worsening future politics.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'minor charm',
      currencyBandCp: [300, 1000],
      permanentUnlock: 'forest passage, village favor, or recurring border conflict hook',
    },
  },
  glass_orchard_masquerade: {
    campaignOrder: 5,
    tier: 1,
    recommendedLevel: [2, 4],
    prerequisites: [['ashveil_keep', 'saltblood_mines']],
    threatProfile: 'social stealth',
    combatExpectation: 'avoidable',
    difficultyNotes: 'Keep the challenge in identity, timing, secrets, and collateral consequences. Combat should be brief and costly in reputation.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: '+1 light weapon only on darker route',
      currencyBandCp: [500, 1500],
      permanentUnlock: 'noble contact, assassin contact, or political leverage',
    },
  },
  elemental_crucible: {
    campaignOrder: 6,
    tier: 2,
    recommendedLevel: [3, 5],
    prerequisites: [['ashveil_keep', 'saltblood_mines']],
    threatProfile: 'elemental trial',
    combatExpectation: 'optional',
    difficultyNotes: 'This is the first major character-power adventure. Tests should challenge identity and restraint more than raw damage output.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 1,
      maxGearTier: 'minor relic',
      currencyBandCp: [0, 750],
      permanentUnlock: 'once-between-rests elemental ability',
    },
  },
  greywash_bandit_crown: {
    campaignOrder: 7,
    tier: 2,
    recommendedLevel: [3, 6],
    prerequisites: [['thorn_treaty', 'blackroot_hollow']],
    threatProfile: 'multi-site faction',
    combatExpectation: 'expected',
    difficultyNotes: 'Escalate through camps, logistics, and morale before the leader. Peaceful dispersal should grant comparable long-term value to bounty hunting.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: '+1 ranged weapon or cache access',
      currencyBandCp: [1000, 3000],
      permanentUnlock: 'farmer militia, county favor, black-market route, or bandit intelligence network',
    },
  },
  sunken_spire: {
    campaignOrder: 8,
    tier: 2,
    recommendedLevel: [4, 6],
    prerequisites: [['elemental_crucible', 'greywash_bandit_crown']],
    threatProfile: 'exploration lore',
    combatExpectation: 'optional',
    difficultyNotes: 'Pressure should come from water, time, ghosts, and archive choices. Reward preservation and rescue as much as treasure extraction.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'minor relic',
      currencyBandCp: [500, 2000],
      permanentUnlock: 'elven records, ghostly blessing, or black-market archive contact',
    },
  },
  merrin_abbey_plague_bells: {
    campaignOrder: 9,
    tier: 2,
    recommendedLevel: [4, 7],
    prerequisites: [['blackroot_hollow', 'thorn_treaty']],
    threatProfile: 'healing spiritual',
    combatExpectation: 'avoidable',
    difficultyNotes: 'Make supplies, triage, names of the dead, and ritual timing matter. Fighting grief should be a fallback, not the best solution.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'divine SP charm',
      currencyBandCp: [200, 1500],
      permanentUnlock: 'abbey healer network or spiritual reputation',
    },
  },
  drowned_market: {
    campaignOrder: 10,
    tier: 2,
    recommendedLevel: [4, 7],
    prerequisites: [['sunken_spire', 'merrin_abbey_plague_bells']],
    threatProfile: 'bargain supernatural',
    combatExpectation: 'dangerous',
    difficultyNotes: 'The main risk is bargain cost. Push memory, identity, law, and debt consequences instead of ordinary loot-room pacing.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'minor relic',
      currencyBandCp: [500, 2500],
      permanentUnlock: 'recurring ghost market access or tide-law contact',
    },
  },
  clockwork_conservatory: {
    campaignOrder: 11,
    tier: 3,
    recommendedLevel: [6, 8],
    prerequisites: [['sunken_spire', 'greywash_bandit_crown']],
    threatProfile: 'mechanical puzzle',
    combatExpectation: 'avoidable',
    difficultyNotes: 'Escalate with synchronized mechanisms, countdown pressure, and ethical choices about sentient machines.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'major tool or clockwork relic',
      currencyBandCp: [1000, 3500],
      permanentUnlock: 'artificer patronage or conservatory research access',
    },
  },
  obsidian_vault: {
    campaignOrder: 12,
    tier: 3,
    recommendedLevel: [6, 9],
    prerequisites: ['elemental_crucible', ['greywash_bandit_crown', 'sunken_spire']],
    threatProfile: 'elemental dungeon',
    combatExpectation: 'expected',
    difficultyNotes: 'The Fire Core should feel valuable and hazardous. Direct fights are fairer here, but heat, fatigue, and unstable terrain should matter.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 1,
      maxGearTier: '+1 weapon or heat-resistant armor',
      currencyBandCp: [1000, 4000],
      permanentUnlock: 'fire-core consequence, smithing access, or volcanic travel protection',
    },
  },
  iron_colosseum: {
    campaignOrder: 13,
    tier: 3,
    recommendedLevel: [6, 9],
    prerequisites: [['greywash_bandit_crown', 'obsidian_vault']],
    threatProfile: 'arena social combat',
    combatExpectation: 'expected',
    difficultyNotes: 'Balance arena victories with escape, revolt, crowd manipulation, and illegal-sale evidence so non-bruisers still have routes to freedom.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 3,
      maxGearTier: '+1 martial reward',
      currencyBandCp: [1200, 4000],
      permanentUnlock: 'freedom papers, gladiator reputation, or arena contact network',
    },
  },
  brass_plague_tinkertown: {
    campaignOrder: 14,
    tier: 3,
    recommendedLevel: [7, 10],
    prerequisites: [['clockwork_conservatory', 'obsidian_vault']],
    threatProfile: 'clockwork outbreak',
    combatExpectation: 'expected',
    difficultyNotes: 'Use conversion infection, civic panic, and workshop problem solving. Destroying machines should be viable but less rewarding than curing or reprogramming.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 3,
      maxGearTier: '+1 tool or schematic relic',
      currencyBandCp: [1500, 5000],
      permanentUnlock: 'Tinkertown workshop access or machine ally',
    },
  },
  mirror_war_saint_orra: {
    campaignOrder: 15,
    tier: 3,
    recommendedLevel: [7, 10],
    prerequisites: ['elemental_crucible', ['glass_orchard_masquerade', 'drowned_market']],
    threatProfile: 'identity judgment',
    combatExpectation: 'optional',
    difficultyNotes: 'The hard part is judgment under time pressure. Combat with the Silver Contrary is an option, but truth, mercy, and precedent should be just as potent.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 1,
      maxGearTier: '+1 light weapon or truth relic',
      currencyBandCp: [500, 3000],
      permanentUnlock: 'reflection ally, identity leverage, or Saint Orra blessing',
    },
  },
  astral_sky: {
    campaignOrder: 16,
    tier: 4,
    recommendedLevel: [9, 12],
    prerequisites: [['clockwork_conservatory', 'mirror_war_saint_orra']],
    threatProfile: 'planar traversal',
    combatExpectation: 'dangerous',
    difficultyNotes: 'Failure should move position, split resources, or strand allies before it simply deals damage. Gravity and timing should be the real boss.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 2,
      maxGearTier: 'major astral relic',
      currencyBandCp: [1500, 6000],
      permanentUnlock: 'sky-route access, prophecy clue, or gravity protection',
    },
  },
  frostfire_crypt: {
    campaignOrder: 17,
    tier: 4,
    recommendedLevel: [10, 13],
    prerequisites: [['merrin_abbey_plague_bells', 'obsidian_vault']],
    threatProfile: 'undead elemental boss',
    combatExpectation: 'expected',
    difficultyNotes: 'This should be a late heroic delve. Cold, fire, undead control, and artifact temptation should stack pressure across the crypt.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 3,
      maxGearTier: 'major elemental relic',
      currencyBandCp: [2000, 7000],
      permanentUnlock: 'Frostfire Heart consequence, frost ward, or scholar patronage',
    },
  },
  harvest_hill_hunger: {
    campaignOrder: 18,
    tier: 4,
    recommendedLevel: [10, 14],
    prerequisites: ['thorn_treaty', ['frostfire_crypt', 'mirror_war_saint_orra', 'brass_plague_tinkertown']],
    threatProfile: 'mythic moral horror',
    combatExpectation: 'dangerous',
    difficultyNotes: 'The Hunger should be beyond a normal monster. Contracts, sacrifice, famine math, and moral compromise should be the primary difficulty.',
    rewardBudget: {
      skillRanks: 2,
      trainingSlots: 1,
      maxGearTier: 'major pact relic',
      currencyBandCp: [500, 5000],
      permanentUnlock: 'harvest pact resolution, food security, corruption hook, or village loyalty',
    },
  },
};

export const ADVENTURE_CHOICE_DIFFICULTY_CURVES = {
  1: ['novice', 'novice', 'professional'],
  2: ['novice', 'professional', 'professional'],
  3: ['professional', 'professional', 'veteran'],
  4: ['professional', 'veteran', 'legendary'],
};
